...
async function connectToWhatsApp() {
  ...
  const socket = makeWASocket({
   ...
   if (!socket.authState.creds.registered) {
      const phoneNumber = await question(`\nEscribe tú número de WhatsApp:\nEx: ${clc.bold("595994966449")}\n/> `);
      const code = await socket.requestPairingCode(phoneNumber);
      console.log(`Tu codigo de conexión es: ${clc.bold(code)}\n`);
      console.log(`Abre tu WhatsApp, ve a ${clc.bold("Dispositivos vinculados >  vincular un dispositivo > vincular usando el numero de teléfono.")}`)
   }
